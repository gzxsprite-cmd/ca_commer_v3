...
function canDownloadWatermark(row) {
...
export default {
  ...
  async render({ query, state }) {
    ...
    return `
      ...
      <section class="focus-panel" style="margin-top:10px;">
        ...
        ${
          canDownloadWatermark(row)
            ? `<div class="actions" style="margin-top:8px;">
                <button class="secondary" id="download-watermark">下载带水印PDF</button>
              </div>`
            : ""
        }
      </section>
  ...
  async bind({ state, query }) {
    ...
    document.getElementById("download-watermark")?.addEventListener("click", () => {
      const a = document.createElement("a");
      a.href = row.watermarked_pdf_path || DRAFT_PDF;
      a.download = (row.watermarked_pdf_path || DRAFT_PDF).split("/").pop() || "watermarked.pdf";
      a.click();
    });
